...
CLI (`brain`) para inicializar y mantener "cerebros" (grafos de conocimiento vía [graphify](https://pypi.org/project/graphifyy/)) para un negocio o cliente: desde cero fuentes de datos hasta un grafo sincronizado y conectado a Claude Code / Claude Desktop.
...
Instalación editable: los cambios en `src/brainiphy_cli/*.py` se aplican de inmediato, sin reinstalar. El binario `brain` queda junto a `graphify` (mismo bin dir de `--user`). Si no aparece en el PATH:

```bash
python3.11 -c "import site,pathlib; print(pathlib.Path(site.getuserbase())/'bin')"
```

y agregar esa ruta al `PATH`, o invocar `brain` por ruta completa.

Verificar que `brain` y `graphify` resuelven al mismo intérprete:
...
```bash
brain new-connector ~/clientes/acme hubspot --interval-minutes 30
```
...
2. Si el conector necesita credenciales:
   ```bash
   brain secret set graphify-acme-hubspot
   ```
...
```bash
brain sync ~/clientes/acme
brain sync ~/clientes/acme --dry-run
```

Este es el comando que también dispara el LaunchAgent programado (ver `brain schedule`), y el que se puede pedir correr manualmente ("sincronizá el CRM ahora").
...
```bash
brain connect-claude ~/clientes/acme --desktop --trust-desktop
```

### `brain schedule [project] --interval-minutes N [--load]`

Genera un LaunchAgent (`~/Library/LaunchAgents/com.graphify.sync.<slug>.plist`) que corre `brain sync <project>` cada N minutos.
...
```bash
brain schedule ~/clientes/acme --interval-minutes 15 --load
```
...
```bash
brain secret set graphify-acme-hubspot
brain secret get graphify-acme-hubspot   # debugging
```
