...
Source lives in `src/cerebro_cli/` (this directory, global — installed once, used everywhere). If `cerebro --help` fails, (re)install it:
```
...
Every project-level file (`connectors/registry.yaml`, `connectors/<name>/sync.py`) is generated by `cerebro`, not hand-copied — `connector_template.py`'s contract imports `cerebro_cli.frontmatter` / `cerebro_cli.keychain` as a real installed package, no path hacking.
