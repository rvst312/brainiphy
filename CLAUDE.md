...
The menu (bare `brain`) is the same problem one layer down — it reads raw keypresses. Drive `menu.run()` with
...
**`src/brainiphy_cli/menu.py`** — bare `brain` (and `brain menu`): the navigable menu, the second front door for a human who should not have to remember `new-connector --preset gohighlevel --var LOCATION_ID=…`. Owns no operations — every entry calls the same `project.py`/`sync.py`/`wizard.py` function the equivalent command calls, and the source-adding entries call `wizard.add_*` directly (which is why those are public rather than `_`-prefixed). What it adds is discovery and ordering. Notes:
